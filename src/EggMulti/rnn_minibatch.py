...
def test_eeg(multiple_out=False, n_epochs=1000, optimizer='cg'):
    """ Test RNN with binary outputs. """
    n_hidden = 6
    n_in = 1
    n_out = 5
    n_seq = 10 
    traindata, targets = Load_data();   

    model = MetaRNN(n_in=n_in, n_hidden=n_hidden, n_out=n_out,
                    learning_rate=0.005, learning_rate_decay=0.999,
                    n_epochs=n_epochs, batch_size=n_seq, activation='tanh',
                    output_type='binary')
    model.fit(traindata, targets, validate_every=100, compute_zero_one=True,
              optimizer=optimizer)    
...
if __name__ == "__main__":
    ...
    test_eeg(optimizer='sgd', n_epochs=1000)
