...
class RNN(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
        ...
        elif self.output_type == 'eeg':
            #self.p_y_given_x = self.softmax(self.y_pred)
            #self.y_out = T.argmax(self.p_y_given_x, axis=-1)
            #self.loss = lambda y: self.eegloss(y)
            self.p_y_given_x = T.nnet.sigmoid(self.y_pred[-1])  # apply sigmoid
            self.y_out = T.round(self.p_y_given_x)  # round to {0,1}
            self.loss = lambda y: self.nll_binary(y)            
        else:
    ...
    def eegloss(self, y):
        #return T.mean((self.y_pred[-1] - y) ** 2)
        #return T.mean(T.nnet.binary_crossentropy(self.y_pred[-1], y))
        #print y.shape
...
def test_binary(multiple_out=False, n_epochs=1600):
...
def Load_data():
    ...
    traindata = np.random.randn(samplelen, 6, 1)
    #traindata = np.asarray(mlpclassify.SX)
    targets = np.zeros((samplelen, 1, 5))
    randarray = np.arange(samplelen)
    np.random.shuffle(randarray)
    for i in range(samplelen):
        r = randarray[i]
        targets[i][0][mlpclassify.SY[r]-1] = 1
        for j in range(6):
            traindata[i][j][0] = mlpclassify.SX[r][j]
    return traindata, targets

def bit2num(blabel):
    for i in range(blabel.shape[0]):
        if blabel[i] == 1:
            return i
    return  -1
...
def test_eeg(multiple_out=False, n_epochs=1000):
    """ Test RNN with binary outputs. """
    n_hidden = 6
    n_in = 1
    n_out = 2
    eegdata = PreProcessing()
    traindata, targets = eegdata.GetData();   
    #print traindata[0]
    #traindata = eegdata.data_x
    #targets = eegdata.data_y

    #print traindata.shape(), targets.shape()
    
    model = MetaRNN(n_in=n_in, n_hidden=n_hidden, n_out=n_out,
                    learning_rate=0.001, learning_rate_decay=0.999,
                    n_epochs=n_epochs, activation='tanh', output_type='binary')
    print model.fit(traindata, targets, validation_frequency=1000)
    model.save(fpath='.', fname="v1.pkl")
    model.load("v1.pkl")
    calculater_result(model, traindata, targets)
